...
const TextAreaInput = ({ blok, register, errors }) => {
    return(
        <SbEditable content={blok}>
            <div className={`${InputSizeMap(blok.size)} ${ContainerClasses}`}>
                ...
                <textarea  {...register(blok.name, { required: blok.required })} className={`${InputClasses}`} name={blok.name} id={blok.name} rows="5" placeholder={blok.placeholder}></textarea>
