...
const EmailInput = ({ blok, register, errors }) => {
    return(
        <SbEditable content={blok}>
            <div className={`${InputSizeMap(blok.size)} ${ContainerClasses}`}>
                ...
                <input {...register(blok.name, { required: blok.required })} className={`${InputClasses}`} type="email" name={blok.name} id={blok.name} placeholder={blok.placeholder} />
